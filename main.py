...
async def home(request: Request):
    ...
    return templates.TemplateResponse("home.html", {
        "request": request,
        "title": "Knowledge Gains - Science-based Weightlifting"
    })
...
async def dashboard(request: Request, user: Dict = Depends(get_current_user)):
    ...
    result = supabase.table("workout_programs").select("*").eq("user_id", user["id"]).eq("is_active", True).execute()
    current_program = result.data[0] if result.data else None
    
    # Get recent workouts
    recent_workouts = []
    if current_program:
        workouts_result = supabase.table("workouts").select("*").eq("program_id", current_program["id"]).order("workout_date", desc=True).limit(5).execute()
...
async def view_program(request: Request, program_id: str, user: Dict = Depends(get_current_user)):
    """View a specific workout program"""
    
    # Get program from database
    result = supabase.table("workout_programs").select("*").eq("id", program_id).eq("user_id", user["id"]).execute()
    ...
    return templates.TemplateResponse("program_view.html", {
        "request": request,
...
async def workout_session(request: Request, workout_id: str, user: Dict = Depends(get_current_user)):
    """Interactive workout session page"""
    
    # Get workout from database
    result = supabase.table("workouts").select("*").eq("id", workout_id).eq("user_id", user["id"]).execute()
    ...
    if workout["week_number"] > 1:
        prev_result = supabase.table("exercise_sets").select("*").eq("user_id", user["id"]).execute()
        previous_sets = prev_result.data
    
    return templates.TemplateResponse("workout_session.html", {
        "request": request,
...
async def update_set(update: WorkoutUpdate, user: Dict = Depends(get_current_user)):
    ...
    result = supabase.table("exercise_sets").select("*").eq("workout_id", update.workout_id).eq("exercise_name", update.exercise_name).eq("set_number", update.set_number).execute()
    ...
    if result.data:
        # Update existing set
        supabase.table("exercise_sets").update(set_data).eq("id", result.data[0]["id"]).execute()
    ...
    if update.weight_kg and update.reps_completed:
        await check_personal_record(user["id"], update.exercise_name, update.weight_kg, update.reps_completed)
...
async def modify_exercise(modification: ExerciseModification, user: Dict = Depends(get_current_user)):
    """Request modification of an exercise using AI"""
    
    # Get current workout data
    workout_result = supabase.table("workouts").select("*").eq("id", modification.workout_id).execute()
    ...
    result = await coordinator.modify_workout(target_exercise, modification.modification_request)
    ...
    if result["success"]:
        ...
        supabase.table("workouts").update({"exercises": modified_exercises}).eq("id", modification.workout_id).execute()
...
async def workout_progress_component(workout_id: str, user: Dict = Depends(get_current_user)):
    """HTMX component for workout progress"""
    
    # Get completed sets
    sets_result = supabase.table("exercise_sets").select("*").eq("workout_id", workout_id).eq("completed", True).execute()
    completed_sets = len(sets_result.data)
    
    # Get total sets from workout
    workout_result = supabase.table("workouts").select("exercises").eq("id", workout_id).execute()
    total_sets = 0
...
async def exercise_card_component(workout_id: str, exercise_name: str, user: Dict = Depends(get_current_user)):
    """HTMX component for individual exercise card"""
    
    # Get workout and exercise data
    workout_result = supabase.table("workouts").select("*").eq("id", workout_id).execute()
    workout = workout_result.data[0]
    ...
    prev_sets = supabase.table("exercise_sets").select("weight_kg").eq("user_id", user["id"]).eq("exercise_name", exercise_name).order("created_at", desc=True).limit(1).execute()
    ...
    return templates.TemplateResponse("components/exercise_card.html", {
        "exercise": exercise,
...
async def check_personal_record(user_id: str, exercise_name: str, weight_kg: float, reps: int):
    ...
    prev_record = supabase.table("progression_tracking").select("estimated_1rm").eq("user_id", user_id).eq("exercise_name", exercise_name).order("measurement_date", desc=True).limit(1).execute()
